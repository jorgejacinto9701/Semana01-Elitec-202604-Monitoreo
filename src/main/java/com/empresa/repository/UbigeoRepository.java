...
public interface UbigeoRepository extends JpaRepository<Ubigeo, Integer> {
    ...
    @Query("select distinct e.departamento from Ubigeo e order by e.departamento")
    public abstract List<String> listaDepartamento();

    @Query("select distinct e.provincia from Ubigeo e where e.departamento = ?1 order by e.provincia")
    public abstract List<String> listaProvincia(String departamento);

    @Query("select e from Ubigeo e  where e.departamento = ?1 and  e.provincia = ?2 order by e.distrito")
